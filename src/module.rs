...
impl FunctionBuilder {
    ...
    pub fn write<'a>(&mut self, op: LinearOp<'a>) {
        ...
        match op {
            ...
            LinearOp::Normal(op) => match op {
                ...
                NormalOp::Br{has_arg, relative_depth} => {
                    ...
                    write_var_u32(ast, relative_depth);
                }
                ...
                NormalOp::BrTable{has_arg, target_data, default} => {
                    ...
                    write_var_u32(ast, (target_data.len() / 4) as u32);
                    ast.extend(target_data);
                    write_u32(ast, default);
                }
                ...
                NormalOp::Const(Dynamic::Int32(v)) => {
                    ast.push(0x10);
                    write_var_i32(ast, unsafe { mem::transmute(v) })
                }
                NormalOp::Const(Dynamic::Int64(v)) => {
                    ast.push(0x11);
                    write_var_i64(ast, unsafe { mem::transmute(v) })
                }
                NormalOp::Const(Dynamic::Float32(v)) => {
                    ast.push(0x13);
                    write_u32(ast, unsafe { mem::transmute(v) })
                }
                NormalOp::Const(Dynamic::Float64(v)) => {
                    ast.push(0x12);
                    write_u64(ast, unsafe { mem::transmute(v) })
                }
                NormalOp::Call{argument_count, index} => {
                    ast.push(0x16);
                    write_var_u32(ast, argument_count);
                    write_var_u32(ast, index.0 as u32);
                }
